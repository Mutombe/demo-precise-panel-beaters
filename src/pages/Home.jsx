  ...
  return (
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-neutral-950">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence mode="sync">
          <motion.img
            ...
            alt="Precise Panel Beaters workshop"
      ...
      <div className="absolute right-5 sm:right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrentSlide(i)} className={`w-[3px] transition-all duration-700 ${i === currentSlide ? 'h-10 bg-orange-500' : 'h-4 bg-white/20 hover:bg-white/40'}`} aria-label={`Slide ${i + 1}`} />
        ))}
      </div>
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="text-orange-400 text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-8" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero?.badge || 'Factory-Grade Panel Restoration'}
        </motion.p>

        <div className="overflow-hidden">
          {['FACTORY', 'FINISH', 'GUARANTEED.'].map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
              <h1
                className={`font-heading leading-[0.88] tracking-tight ${line === 'FINISH' ? 'bg-gradient-to-r from-orange-500 via-orange-400 to-orange-600 bg-clip-text text-transparent' : 'text-white'}`}
                style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: line === 'FINISH' ? 900 : 300 }}
              >
                {line}
              </h1>
            </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 1.2 }} className="flex items-center gap-3 mt-8">
          ...
          <p className="text-white/30 text-xs sm:text-sm uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>
            31 Reviews &middot; 4.6 Stars &middot; Kelvin Rd, Harare
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.3 }} className="flex items-center gap-3 mt-6">
          <div className="flex items-center gap-2 bg-orange-500/10 border border-orange-500/30 px-4 py-2">
            <ShieldCheck size={18} weight="fill" className="text-orange-500" />
            <span className="text-orange-400 text-xs uppercase tracking-[0.15em] font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>12-Month Warranty on All Work</span>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.5 }} className="flex flex-wrap gap-4 mt-8">
          <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-orange-500 text-neutral-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:bg-orange-400 hover:shadow-xl hover:shadow-orange-500/20" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero?.ctaPrimary || 'Get Free Quote'}
            <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
          <Link to="/projects" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:border-orange-500/50 hover:text-orange-400 hover:bg-white/5" style={{ fontFamily: 'var(--font-sans)' }}>
...
function MarqueeTicker() {
  ...
  return (
    <section className="bg-neutral-950 border-y border-orange-500/10 py-5 sm:py-6 overflow-hidden">
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-orange-500/80 font-heading text-lg sm:text-2xl tracking-wider font-bold uppercase">{item}</span>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-orange-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Transformations</p>
    ...
    <section ref={ref} className="bg-neutral-950 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-orange-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Expertise</p>
              <h2 className="font-heading text-white leading-[0.92] font-bold uppercase" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                What We <span className="text-orange-500">Repair</span>
              </h2>
            </div>
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.08 * i }} className={i === 0 ? 'sm:col-span-2 lg:col-span-2' : ''}>
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`} className={`group relative block overflow-hidden ${i === 0 ? 'aspect-[16/9] sm:aspect-[2/1]' : 'aspect-[3/4]'}`}>
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-orange-500/15 font-heading text-6xl sm:text-7xl font-bold leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats?.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: i * 0.12 }} className="text-center relative">
              <div className="font-heading text-orange-500 leading-none font-bold" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)', textShadow: '0 0 40px rgba(249,115,22,0.15)' }}>
              ...
              {i < (stats?.length || 0) - 1 && <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 h-16 w-[1px] bg-gradient-to-b from-transparent via-orange-500/15 to-transparent" />}
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[2px] bg-orange-500 mb-6" />
            <p className="text-orange-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Legacy</p>
            <h2 className="font-heading text-white leading-[0.95] font-bold uppercase mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              Precise <span className="text-orange-500">Panel Beaters</span>
            </h2>
            <p className="text-white/50 text-sm sm:text-base leading-relaxed mb-6 max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              Founded on a commitment to factory-grade vehicle restoration, Precise Panel Beaters has earned the trust of Harare's motorists through relentless attention to detail and uncompromising quality standards.
            </p>
            ...
            <div className="flex gap-10 sm:gap-16">
              <div>
                <div className="text-orange-500 font-heading text-3xl sm:text-4xl font-bold leading-none">4.6</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Star Rating</div>
              </div>
              <div>
                <div className="text-orange-500 font-heading text-3xl sm:text-4xl font-bold leading-none">31</div>
                <div className="text-white/30 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Reviews</div>
              </div>
              <div>
                <div className="text-orange-500 font-heading text-3xl sm:text-4xl font-bold leading-none">12mo</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            <div className="relative">
              ...
              <div className="absolute -top-4 -right-4 sm:-right-6 bg-orange-500 text-neutral-950 p-5 sm:p-7 shadow-2xl">
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.15 }}>
            ...
            <h2 className="font-heading text-white leading-[0.95] font-bold uppercase mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-orange-500">Precise</span>
            </h2>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="flex items-center gap-0.5 mt-1">{[...Array(t.rating || 5)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-orange-500" />)}</div>
              </div>
            </motion.div>
          </AnimatePresence>
    ...
    <section ref={ref} className="relative py-24 sm:py-32 overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="relative z-10 max-w-3xl mx-auto px-5 sm:px-8 text-center">
        ...
        <h2 className="font-heading text-white leading-[0.92] font-bold uppercase mb-6" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
          Your Vehicle Deserves <span className="text-orange-500">Precision</span>
        </h2>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/contact" className="group inline-flex items-center justify-center gap-3 bg-orange-500 text-neutral-950 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-orange-400 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
            Get Free Quote <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
          <a href="https://wa.me/263719692306" target="_blank" rel="noopener noreferrer" className="group inline-flex items-center justify-center gap-3 border border-green-500/40 text-green-400 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-green-500/10 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
